...
const Marketplace = () => {
  ...
  const handleCheckout = async () => {
    ...
    if (deliveryDetails.paymentMethod === 'card') {
      ...
      if (deliveryDetails.cardDetails.cardNumber.length !== 16) {
        toast.error('Please enter a valid 16-digit card number');
        return;
      }
      
      // Expiry date validation
      if (!isValidExpiryDate(deliveryDetails.cardDetails.expiryDate)) {
        toast.error('Please enter a valid expiry date (MM/YY)');
        return;
      }
      
      // CVV validation
      if (deliveryDetails.cardDetails.cvv.length < 3 || deliveryDetails.cardDetails.cvv.length > 4) {
    ...
    try {
      // Create a single order request with all necessary information
      const orderData = {
        customerId: user?.id || '',
        ...
        // Add delivery details to the order request
        address: deliveryDetails.address,
        scheduledDate: deliveryDetails.scheduledDate
      };

      // Create the order with the backend endpoint that now handles everything
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-green-500"
                    />
                    {deliveryDetails.scheduledDate && new Date(deliveryDetails.scheduledDate) < new Date() && (
                      <p className="text-red-500 text-sm mt-1">Please select a future date and time</p>
                    )}
                  </div>
                </div>
  ...
  <div className="mt-2 space-y-2 p-2 border border-gray-200 rounded-md">
    <div>
      ...
      <input
        type="text"
        name="cardNumber"
        value={formatCardNumber(deliveryDetails.cardDetails.cardNumber)}
        onChange={(e) => {
      ...
      />
      {deliveryDetails.cardDetails.cardNumber.length < 16 && (
    ...
    <div className="flex space-x-3">
      <div className="w-1/2">
        ...
        <input
          type="text"
          name="expiryDate"
          value={formatExpiryDate(deliveryDetails.cardDetails.expiryDate)}
          onChange={(e) => {
          ...
          placeholder="MM/YY"
        />
        {!isValidExpiryDate(deliveryDetails.cardDetails.expiryDate) && (
      ...
      <div className="w-1/2">
        ...
        <input
          ...
          placeholder="123"
        />
        {deliveryDetails.cardDetails.cvv.length < 3 && (
          <p className="text-red-500 text-sm mt-1">CVV must be 3-4 digits</p>
        )}
      </div>
    </div>
  </div>
